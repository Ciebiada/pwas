...
export class EpubRenderer {
    ...
    async displaySpineIndex(
        ...
        try {
            ...
            const { html, attributes } = await this.processHtml(htmlContent, manifestItem.href);
    ...
    private async resolveCombinedStyles(doc: Document, baseHref: string): Promise<string> {
        const UA_STYLES = `
        ...
        for (const tag of styleTags) {
            combinedCss += (await this.resolveUrlsInCss(tag.textContent || '', baseHref)) + '\n';
        }
        ...
        for (const link of links) {
            ...
            if (href) {
                ...
                try {
                    ...
                    console.warn(`[Renderer] Failed to load stylesheet: ${resolvedPath}`, e);
                }
            }
        }
    ...
    private async resolveUrlsInCss(css: string, baseHref: string): Promise<string> {
        ...
        for (const match of matches) {
            ...
            if (cachedUrl) {
                ...
                try {
                    ...
                    console.warn(`[Renderer] Failed to resolve CSS URL: ${resolvedPath}`, e);
                }
            }
        }
        return resolvedCss;
    ...
    private applyStyles(preserveTransform: boolean = false) {
        ...
        const previousTransform = preserveTransform ? this.contentElement.style.transform : '';
    ...
    getCurrentLocation(basicOnly: boolean = false): any {
        ...
        const chapterPercentage = this.totalPages > 0 ? this.currentPage / this.totalPages : 0;
